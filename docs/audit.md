...
The audit also runs `python3 -m py_compile`, `bash -n build.sh`, all three ACME targets, `shasum -a 256 -c SHA256SUMS.txt`, and `git diff --check`. The README, changelog, build names, checksum manifest, source path, and release notes all agree on the current release identity and artifact names.

## Live emulator check

The built `euro-pulsegrid.prg` was launched in VICE. The normal visualizer reached both a lower-intensity detail state and a high-energy colour peak without a CPU jam; captured frames are stored in `assets/` and displayed in the README. See `tools/audit_release.py` for the executable structural checks.
